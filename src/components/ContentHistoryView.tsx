...
export default function ContentHistoryView({ folder, onNavigate, onPreviewFile }: ContentHistoryViewProps) {
    ...
    return (
      <div onClick={() => onPreviewFile(item)} className="group flex items-start gap-4 p-4 rounded-[20px] bg-card-bg border border-transparent hover:border-card-border hover:bg-card-hover transition-all cursor-pointer relative">
        ...
        <div className="flex-1 min-w-0 pr-4">
          <h3 className="text-sm font-medium truncate mb-0.5">{item.name}</h3>
        ...
        <div className="flex flex-col items-end gap-2">
            ...
            <button onClick={(e) => handleDelete(e, item.id, item.name)} className="p-2 text-muted-foreground/20 hover:text-red-500 opacity-0 group-hover:opacity-100 transition-all">
                <Trash2 size={16} />
            </button>
        </div>
      </div>
  ...
  const MediaPreviewItem = ({ item }: { item: any }) => {
    ...
    return (
      <div onClick={() => setSelectedItem(displayItem)} className="relative group aspect-square rounded-[24px] overflow-hidden bg-card-bg border border-card-border hover:border-primary/50 transition-all cursor-pointer">
  ...
  if (selectedItem) {
    return <DocumentViewer item={selectedItem} onBack={() => setSelectedItem(null)} onDelete={async (item) => { await deleteFileById(item.id); await loadAllData(); setSelectedItem(null); }} />;
  }
                ...
                return (
                  <section key={cat}>
                    <div className="flex items-center justify-between mb-4 px-2">
                      ...
                      <button onClick={() => onNavigate(cat)} className="h-8 w-8 flex items-center justify-center bg-card-bg border border-card-border rounded-full transition-all"><ChevronRight size={18} /></button>
                    </div>
                    {isMedia ? (
